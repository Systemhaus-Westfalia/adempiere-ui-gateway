...
The stack is modular and service-oriented:

1. **Service Definition Files**: Each service is defined in a separate docker-compose YAML file (e.g., `01a-postgres_service_with_ports.yml`, `17d-ui_gateway_service_standard.yml`) in `docker-compose/` directory
2. **Stack Configurations**: Pre-defined service combinations for different purposes (auth, cache, develop, storage, vue, standard/default)
3. **Dynamic Composition**: The `start-all.sh` script assembles a `docker-compose.yml` file by concatenating the appropriate service files based on the selected stack mode
...
├── start-all.sh              # Start stack script
├── stop-all.sh               # Stop stack script
├── stop-and-delete-all.sh    # Complete cleanup script
├── 01a-postgres_service_*.yml   # Individual service definitions
├── 17d-ui_gateway_service_*.yml # nginx gateway service definitions
├── docker-compose-*.yml      # Legacy monolithic files (deprecated)
├── postgresql/
